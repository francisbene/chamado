	...
	if (!gx.lang.emptyObject(vStruct) && !gx.lang.emptyObject(gx.rtPicture(vStruct, Ctrl)) && !gx.lang.emptyObject(vStruct.type)) {
		try {
			switch (vStruct.type) {
				case 'int':
				case 'decimal':
					value = Number.isNaN(Number(value)) ? gx.num.extractValue(gx.rtPicture(vStruct, Ctrl), gx.thousandSeparator, value) : value;
...
gx.dom_i =(function($){
	...
	findRadioContainer: function (Ctrl) {
		return (Ctrl.parentNode && Ctrl.parentNode.tagName == 'SPAN') ? 
			Ctrl.parentNode : gx.dom.findParentByTagName(Ctrl, 'table') || gx.dom.findParentByTagName(Ctrl, 'span')||Ctrl.parentNode;
	},
...
gx.evt_i = (function($) {
	...
	onblur: function (Ctrl, gxLastFocusCtrl) {
		...
		if (gx.O.GXLastCtrlId === gxLastFocusCtrl && window.event && window.event.relatedTarget === null) {
			//Last input control with no button below in the form + focus beyond form
			gx.csv.ExecutingOnBlur = true;
			gx.csv.validControls(gxLastFocusCtrl, gxLastFocusCtrl + 1, true, gx.O);
			gx.csv.ExecutingOnBlur = false;
		}
		try {
	...
	onfocus_impl: function (Ctrl, gxFocusCtrl, gxWCP, gxInMasterPage, gxCurrentRow, gxCurrentGrid, gxAddLines) {
		...
		try {
			...
			if (gx.csv.invalidForcedCtrl) {
				var didFocus = gx.fn.setFocusOnError(gx.csv.invalidForcedCtrl.Ctrl.id)
	...
	onchange_impl: function(Ctrl, event, preventValid) {
		...
		var gxO = gx.fn.getControlContext(Ctrl) || gx.O,
			vStruct = gxO.getValidStructFld(Ctrl),
			hasRVFmt = vStruct && vStruct.hasRVFmt,
			onChangeDeferred = $.Deferred();
		if(Ctrl.GXFormatError && !hasRVFmt) {
		...
		if (gx.csv.controlValueHasChanged(Ctrl, CtrlValue, vStruct)) {
			...
			if (vStruct) {
				...
				if (typeof(vStruct.c2v) == 'function')
					vStruct.c2v(sRow);
				gx.fn.call_v2bc(vStruct, gxO, sRow);
			}
			if (!preventValid) {
			...
			gx.fx.obs.notify('gx.validation', null, (function () {
				...
				if (!srvCommand && (gx.grid.drawAtServer || (ctrl && ctrl.nodeName == 'INPUT' && ctrl.type == 'submit'))) {
					...
					var oldDoPost = function() {
							gx.ajax.doPost((gx.http.useNamedParameters(gx.ajax.selfUrl())? 'gxevent=' : '') + gx.ajax.encryptParms(gx.pO, 'gxajaxEvt'), sync, true, gridId, rowId, callback, gxObj);
						fnc_always();
					};
...
gx.csv_i =  (function($) {
	...
	return {
			...
			validateAll: function (gxO) {
				...
				this.validControls(gxO.fromValid, gxO.toValid, false, gxO).done((function (bret, invalidControl) {
					try {
						if (this.invalidControl == null || !gx.csv.stopOnError){
							gxO.fromValid = invalidControl || gxO.toValid;
						}
			...
			validControls: function (FromControl, TargetControl, bForceCheck, gxO, toUpperRowInGrid) {
				...
				var resolve = function (bRet) {
					gxO.endFeedback();
					deferred.resolve(bRet, invalidControl);
				};
				...
				try {
					...
					if (TargetControl > FromControl) {
						...
						for (var i = FromControl; i < TargetControl; i++) {
							...
							if( Control && Control.GXFormatError && Control.GXFormatError === true && !hasRVFmt){
								invalidControl=i;
								break;
							}
			...
			setFormatError: function(Ctrl, error) {
				Ctrl = gx.dom.el(Ctrl);
				if (Ctrl.id) {
					var contains = this.gxFormatErrors[Ctrl.id] !== undefined;
					if (error || typeof(error) == 'undefined') {
						if(!contains){
							this.gxFormatErrors[Ctrl.id] = this.gxFormatErrors.push(Ctrl.id) - 1;
						}
						Ctrl.GXFormatError = true;
					}
					else {
						if(contains){
							this.gxFormatErrors.splice(this.gxFormatErrors[Ctrl.id],1);
						}
			...
			validGridRowChange: function (GridId, GXValidRow, gxO, lastRow) {
				...
				if(gx.csv.anyFormatError()){
					var errors = gx.csv.gxFormatErrors;
					gx.csv.invalidForcedCtrl = gx.csv.invalidForcedCtrl || {};
					gx.csv.invalidForcedCtrl.Ctrl = gx.dom.el(errors[0]);
					gx.csv.invalidForcedCtrl.ForceFocus = true
					gx.fn.setFocus(gx.csv.invalidForcedCtrl.Ctrl);
					deferred.resolve(false);
				} else {
...
(function($) {
	...
	prot.getGridForColl = function (collVarName) {
		var returnGrid = this.getAllGridsForColl(collVarName);
		return returnGrid.length === 0 ? null : returnGrid[0];
	};

	prot.getAllGridsForColl = function (collVarName) {
		var len = this.Grids.length;
		var returnGrids = [];
		for (var i = 0; i < len; i++) {
			var gxGrid = this.Grids[i];
			if (gxGrid.boundedCollName == collVarName)
				returnGrids.push(gxGrid);
		}
		return returnGrids;
	};
	...
	prot.setVariable = function (VarName, Value) {
		...
		if (this.isTransaction() && (VarName == "Gx_mode")) {
			this.setMode();
		}
	...
	prot.inputHasFormatErrors = function (dirtyEventName, grid, row) {
		...
		var self = this;
		var anyError = function (vStruct, pGrid, pRow) {
			var grid = pGrid || grid,
			row = pRow || row;
			if (vStruct) {
				if (grid && row && vStruct.grid == grid) {
					element = gx.fn.getControlGridRef(vStruct.fld, grid, row);
					if (element && element.id && (gx.csv.gxFormatErrors[element.id] !== undefined)) {
						return true;
					}
				}
				else {
		...
		for (i=0, len=inputParms.length; i<len && !anyErr; i++) {
			...
			if (inputParm.fld) {
				...
				if (vStruct) {
					...
					var gridsBC = gx.O.getAllGridsForColl(inputParm.av);
					if (gridsBC) {
						gridsBC.some(gridBC => {
								var GXValidFnc = gx.O.GXValidFnc,
								gridId = gridBC.gridId;
								GXValidFnc.filter(fnc => fnc.grid === gridId).forEach(field => {
								vStruct = this.getValidStructFld(field.fld);
								if(vStruct){
									gridBC.grid.rows.forEach(row => {
										anyErr = anyErr || anyError(vStruct,gridId,row.gxId);
									})
								}
								return anyErr;
							});
	...
	var elementIsFocusable = (function () {
		...
		return function (el, shouldConsiderChildren = false) {
			if (!el) {
				return false;
			}
			var tagName = el.tagName.toLowerCase();
			var arrayChildren = [].slice.call(el.children);

			focusableTagNames['a'] = shouldConsiderChildren

			var childIsFocusable = shouldConsiderChildren && arrayChildren.some(child => 
				elementIsFocusable(child, shouldConsiderChildren)
			)
			return childIsFocusable || (tagName == "input" ? focusableInputTypes[el.type.toLowerCase()] : focusableTagNames[tagName]);
		};
	})();
	...
	var dispatchEvent = function (domEvt, $target, gxO, fld ) {
		if ($target[0] && $target[0].type === 'button' && $target[0].disabled) {
		...
		switch (jsScriptType) {
			...
			default:
				if (!conditionCode || evaluateCode(conditionCode)) {
					...
					if (evtName) {
						var compare1 = evtName.substring(evtName.indexOf('.')+1,evtName.length).toLowerCase();
						var compare2 = domEvt.type.toLowerCase();
						if (compare1.includes('click') && !compare2.includes('click') && domEvt.currentTarget && domEvt.currentTarget.type === 'textarea'){
							return;
						}
				...
				if (vStruct.grid === 0) {
					if (vStruct.fld) {
						...
						try {
							...
							gx.dbg.logEx(e, 'gxfrmutl.js', 'registerCtrlsEventHandlers');
						}
						if (hasEvt) {
							...
							if (!elementIsFocusable($controlElement.get(0), true)) {
								$controlElement.attr("tabindex", "0");
							}
						}
	...
	prot.refreshExoEventControls = function(exoEventName) {
		const methodName = `GLOBALEVENTS.${exoEventName.toUpperCase()}`;
		let parms = (this.EvtParms[methodName]) ? this.EvtParms[methodName][0] : [];
		parms.forEach( p => {
			let vStruct = p.fld ? this.getValidStructFld(p.fld) : null;
			if (vStruct) {
				let vRow = gx.fn.currentGridRowImpl(vStruct.grid);
				if (typeof (vStruct.c2v) == 'function' && typeof (vStruct.val) == 'function') {
					vStruct.val(vRow) && vStruct.c2v(vRow);
					gx.fn.call_v2bc(vStruct, this);
				}
				if (vStruct.isuc === true) {
					(vStruct.getUCInstance(vRow) || vStruct.uc).execC2VFunctions();
				}
			}
		});
	};
	...
	prot.addExoEventHandler = function (exoEventName, handler) {
		var exoEventHandler = function () {
			var oldGxO = gx.O;
			gx.setGxO(this);
			this.refreshExoEventControls( exoEventName);
	...
	return {
		...
		assignPromptFields: function (allParams, cParms) {
			...
			var gxO = gx.O,
				orderedParams = [],
				ctrlIdx = 99999,
				len = Math.min(allParams.length, cParms.length),
				i = 0;
			for (i = 0; i < len; i++) {
		...
		assignAndValidatePromptField: function (obj, gxO) {
			...
			var callback = (function () {
				...
				if (toValid) {
					//Validate ParmCtrl
					if (gx.O.fromValid > toValid) {
						gx.O.fromValid = toValid;
					}
		...
		ext: {													
			...
			resizePopupContentImpl: function (width, height) {				
				...
				try {
				  var ScrollbarWidth = $iFrame[0].scrollWidth - $iFrame[0].contentDocument.scrollingElement.offsetWidth;
					docWidth += ($iFrame.height() < frameDoc.body.offsetHeight)? ScrollbarWidth: 0;
				} catch(e) {}
...
gx.ajax = (function ($) {
	...
	return {
			...
			return {
				...
				refreshOutputParms: function () {
					...
					for (var i = 0, len = parmsMetadata.length; i < len; i++) {
						...
						vStruct = this.getParmVarVStruct(parm)[0];
				...
				getParmVarVStruct: function (parm) {
					...
					if (parm.av) {
						BC = gx.O.getFormBCForVar(parm.av);
					}
					if (BC) {
						validStruct_arr = Object.values(BC).filter( v => v.ctrltype);
					}
					else {
						if (parm.fld !== undefined) {
							...
							if (this.grid && this.row) {
								...
								if (gridObject) {
									...
									if (gridColumn) {
										validStruct_arr.push(this.gxO.getValidStruct(gridColumn.gxId));
									}
								}
							}
						}
					}
					return validStruct_arr;
				},
		...
		doPost: function (ParmString, Synch, disableForm, gridId, rowId, callback, gxO) {
			if (disableForm !== false) {
				gxO.startFeedback();
			}
			gx.fx.obs.notify('gx.onbeforeevent', [ParmString, Synch]);
...
gx.util.balloon = {
		...
		this.show = function (batch) {
			...
			try {
				...
				else if (Control.nodeName === 'INPUT' && Control.type === 'radio')
				{
					...
					Control = gx.dom.findRadioContainer( Control);
				}
				if (!ControlList) {
					ControlList = [Control];
	...
	getNew: function (Id, RowId, SourceElements) {
		...
		try {
			...
			if (Ctrl.nodeName === 'INPUT' && Ctrl.type === 'radio')
			{
				var ControlList = gx.dom.byName(gx.dom.id(Ctrl));
				Ctrl = gx.dom.findRadioContainer( Ctrl);
				if (!Ctrl.id) {
					Ctrl.setAttribute('id', 'BallonAnchor_' + Id);
				}
			}
